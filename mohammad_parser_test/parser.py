...
class LL1:
    ...
    def get_next_valid_token(self):
        try:
            ...
            while token.type in ['COMMENT', 'WHITE_SPACE', 'ERROR']:
                token = self.token_generator.get_next_token()
            return token
        except Exception:
    ...
    def get_token_key(token):
        return (token.lexeme, token.type.name)[token.type in ['NUM', 'ID']]
